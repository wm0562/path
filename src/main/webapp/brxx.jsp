      ...
      <div class="container___1I4jN undefined" style="height: 89vh;">
       <div class="containerHeader___1Lcq6">
        <div id="search" style="float:right;font-size:12px;">
         	输入病人ID或者姓名查询：<input type="text" name="searchcontent" id="searchcontentId" style="border:0.5px solid gray;height:20px;">
			<input type="submit" name="submit" id="searchid" value="搜索">
			</div>
        	<div class="container">
   			 	<form action="/path/addToPatient" method="post" class="form-horizontal" role="form" id="myForm" onsubmit="return ">
            ...
            <!--  定义模态框，过渡效果为淡入，id为myModal,tabindex=-1可以禁用使用tab切换，aria-labelledby用于引用模态框的标题，aria-hidden=true保持模态框在触发前窗口不可见  -->
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body" id="modal-data">
                        <!--  模态框内容，我在此处添加一个表单 -->
                        <form>
                                                                            病人ID： <input type="text" name="pid" style="width: 120px;height: 25px;border: 1px solid gainsboro;"/>
                                                                            姓名： <input type="text" name="pname" style="width: 120px;height: 25px;border: 1px solid gainsboro;"/>
                                                                            性别： <input type="text" name="psex" style="width: 120px;height: 30px;border: 1px solid gainsboro;"/> <br />                                    
                                                                            年龄： <input type="text" name="page" style="width: 120px;height: 25px;border: 1px solid gainsboro;"/>
                                                                            地址： <input type="text" name="paddress" style="width: 120px;height: 25px;border: 1px solid gainsboro;"/>
                                                                             状态：<input type="text" name="status" style="width: 120px;height: 30px;border: 1px solid gainsboro;"/> <br />
                                                                           入院时间： <input type="date" name="entertime" style="width: 150px;height: 30px;border: 1px solid gainsboro;"/>   
                                                                            出院时间： <input type="date" name="outtime" style="width: 150px;height: 30px;border: 1px solid gainsboro;"/>                                                     
                        </form>
                    </div>
...
function getAll(pn){
	$.ajax({
		url:"http://localhost:8080/path/getPatient",
		type:"get",
		data:"pn="+pn,
		success:function(result){
			 ...
			 $.each(emps,function(index,item){
			 	var s="<tr class=\"ant-table-row  ant-table-row-level-0\">"+
                  ...
                  " <td class=\"wpColumn\">"+item.page+"</td>"+
                  " <td class=\"wpColumn\">"+item.paddress+"</td>"+
                  " <td class=\"wpColumn\">"+item.status+"</td>"+
                  " <td class=\"wpColumn\">"+item.entertime+"</td>"+
...
function build_page_nav(result){
	...
	var firstPageLi = $("<li></li>").append($("<a></a>").append("首页").attr("href","brxx.jsp"));
...
$(document).on("click","#look-data",function(){
	...
	//查询出要修改的数据
	$.ajax({
		url:"http://localhost:8080/path/getEveryPatient/"+id,
    		type:"get",
    		success:function(result){
    		...
    		document.getElementsByName("page")[0].value = item[0].page;
    		document.getElementsByName("paddress")[0].value = item[0].paddress;
    		document.getElementsByName("status")[0].value = item[0].status;
    		document.getElementsByName("entertime")[0].value = item[0].entertime;
...
$(document).on("click","#searchid",function(){
	  ...
	  var inputvalue = document.getElementsByName("searchcontent")[0].value;
	    	$.ajax({
	    		url:"http://localhost:8080/path/searchcontentPa/"+inputvalue,
	    		type:"GET",
	    		success:function(result){
	    			 $("#databody").empty();
	    			 var emps=result.extend.pageInfo.list;
